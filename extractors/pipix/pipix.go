...
func (e extractor) Extract(uri string, option extractors.Options) ([]*extractors.Data, error) {
	...
	_, err = request.Client.R().SetHeader("user-agent", "Mozilla/5.0 (iPhone; CPU iPhone OS 11_0 like Mac OS X) AppleWebKit/604.1.38 (KHTML, like Gecko) Version/11.0 Mobile/15A372 Safari/604.1").SetResult(&video).
		Get("https://is.snssdk.com/bds/cell/detail/?cell_type=1&aid=1319&app_name=super&cell_id=" + match[1])
	if err != nil {
		return nil, err
	}

	downAddr := video.Data.Data.Item.OriginVideoDownload.URLList[0].URL

	body, err := request.Client.R().Head(downAddr)
	if err != nil {
		return nil, err
	}
	...
	return []*extractors.Data{
		{
			Site:    "皮皮虾 pipix.com",
			Title:   video.Data.Data.Item.Content,
			Type:    extractors.DataTypeVideo,
			Streams: streams,
			URL:     uri,
			Cover:   video.Data.Data.Item.Cover.URLList[0].URL,
		},
	}, nil
